...
export default async function ProductPage({ params }: { params: { lang: string; slug: string } }) {
  ...
  // Fetch category by its key (product.category_id is the key after migration)
  const { data: category } = await supabase
    .from("categories")
    .select("*")
    .eq("key", product!.category_id)
    .maybeSingle();

  const categoryName = category
    ? params.lang === "ar" && category.name_ar
      ? category.name_ar
      : category.name_en
    : null;
            ...
            <div className="p-8 md:p-12 flex flex-col">
              ...
              <div className="prose prose-gray mb-8 flex-grow">
                <p className="text-gray-600 text-lg leading-relaxed whitespace-pre-wrap">
                  {description ||
                    (params.lang === "ar" ? "لا يوجد وصف." : "No description available.")}
                </p>
              </div>
